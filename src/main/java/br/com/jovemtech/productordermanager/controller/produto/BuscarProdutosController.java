...
public class BuscarProdutosController {
    ...
    @GetMapping
    public ResponseEntity<Page<ProdutoGetDTO>> buscarProdutos(@Parameter(hidden = true) Pageable pageable){
        Page<ProdutoGetDTO> dtos = buscarProdutosUC.execute(pageable);

        if (!dtos.isEmpty()) {
            for (ProdutoGetDTO produto : dtos) {
                Long id = produto.getId();
                produto.add(linkTo(methodOn(BuscarProdutoPorIdController.class)
                        .buscarProdutoPorId(id, pageable)).withSelfRel());
            }
        }
